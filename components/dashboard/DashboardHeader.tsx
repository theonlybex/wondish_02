...
function CouponInput({ onClose }: { onClose: () => void }) {
  ...
  return (
    <div className="px-4 pb-4 pt-1">
      <p className="text-[#8A8D93] text-xs mb-3">Enter a coupon code to unlock access.</p>
    ...
    <header className="h-16 bg-white border-b border-[#E8E7EA] flex items-center justify-between px-5 sm:px-8">
      ...
      <div className="flex items-center gap-3">
        ...
        {plan === "ADMIN" ? (
          ...
          <Link
            href="/pricing"
            className="text-xs font-bold px-2.5 py-1 rounded-full bg-[#F3F2FF] text-[#8A8D93] hover:bg-primary/10 hover:text-primary transition-colors"
          >
            Free → Upgrade
          </Link>
        )}
                ...
                <>
                  <div className="py-1.5">
                    <Link
                      ...
                      className="flex items-center gap-3 px-4 py-2 text-sm text-[#4A4E5C] hover:bg-[#F8F7FA] transition-colors"
                    >
                      <span className="text-base">👤</span> My Profile
                    </Link>
                    <button
                      onClick={() => setShowCoupon(true)}
                      className="w-full flex items-center gap-3 px-4 py-2 text-sm text-[#4A4E5C] hover:bg-[#F8F7FA] transition-colors text-left"
                    >
                      <span className="text-base">🎟️</span> Redeem coupon
                    </button>
                  </div>
                ...
                <>
                  <div className="px-4 pt-3 pb-1 flex items-center gap-2">
                    <button
                      ...
                      className="text-[#8A8D93] hover:text-[#25293C] transition-colors"
                    >
                      ←
                    </button>
                    <p className="text-[#25293C] font-semibold text-sm">Redeem coupon</p>
                  </div>
