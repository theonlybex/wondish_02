        ...
        <div className="flex items-center gap-3">
          <div className="flex -space-x-2">
            {avatars.map((letter, i) => (
              <div
                key={i}
                className={`w-7 h-7 rounded-full ${avatarColors[i]} border-2 border-[#0d1a10] flex items-center justify-center text-white text-[10px] font-bold`}
              >
                {letter}
              </div>
            ))}
          ...
          <span className="text-white/50 text-sm">
            Joined by <span className="text-white font-semibold">10,000+</span> members
          </span>
        </div>
