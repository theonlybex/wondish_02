...
export async function POST(req: Request) {
  ...
  // Only now the in-flight lock: one cook-my-day at a time, so a double-tap
  // cannot generate a second set of PUBLIC recipes (and pay Anthropic twice).
  //
  // This was a rate limit — `rateLimit("ai-cookday-inflight", userId, 1, 90)` —
  // and a rate limit cannot be released. A day that cooked successfully in
  // eight seconds left the user locked out of their own feature for the other
  // eighty-two, told Clara was "already cooking" something that had finished.
  // A lock has a release; that is the whole difference, and it is why the 90
  // seconds below is now a CRASH GUARD rather than a cooldown (lib/in-flight-lock.ts).
  const lock = await acquireInFlight("cookday", userId, 90);
  if (!lock.acquired) {
    return NextResponse.json(
  ...
  // Everything below runs holding the lock, so every exit goes through the
  // finally. A thrown error releases it too: the next click then gets the real
  // failure instead of a busy message about a request that already died.
  try {
