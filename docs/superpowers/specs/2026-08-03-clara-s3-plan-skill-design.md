...
| `plan_swap_dish` ✍️ | Inputs: `menuId`, `recipeId` — the recipeId obtainable only from `plan_alternatives` in this conversation. `validateSwapCandidate` enforces the route's FULL rule set: menu ownership + active version, recipe public, meal-type match, diet-ban pass, ≤50pp macro deviation, same-day family conflict (beverage exemption), same-meal sub-family conflict. (Amended 2026-08-03 during planning: the route carries the family/sub-family rules too — parity includes them.) Typed failures for each rejection so Clara can explain why a candidate was refused. |
