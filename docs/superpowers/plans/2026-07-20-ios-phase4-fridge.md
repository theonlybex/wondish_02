...
> **AMENDED 2026-07-23 (Cycle-5 execution amendment — reconciles this plan with code shipped in Cycles 2-4; supersedes conflicting text below):**
>
> 1. **Ban derivation + allergen filter REUSE `lib/diet-match.ts` (shipped, reviewed, behavior-parity-verified in the Restaurants cycle).** `collectBannedTerms(patient)` is implemented on top of `derivePatientBans` (flatten `allergyNames` + `exactBanned[].name`, lowercase/dedupe) — NOT a fresh hand-rolled union. `applyAllergenFilter` uses `buildDietMatchers` word-boundary matchers (allergy regexes + exact set) against each recipe's name/usesIngredients/missingIngredients/steps — never raw substring (this operationalizes fix 5 of the 2026-07-22 amendment; the "planned extraction" it referenced now exists). `PATIENT_FOOD_MAP_INCLUDE` composes `PATIENT_DIET_INCLUDE` + `mealType: true` rather than redeclaring the graph. `buildFoodMapText` extraction (display text) stays verbatim per Task 1 — the Restaurants-cycle review adjudicated it un-refactorable onto the engine, but it can still MOVE to `lib/food-map.ts`.
> 2. **Client-side freemium is VOID (Cycle-2 architecture decision).** `UsageMeter`, `EntitlementStore`, `PaywallView`, `PaywallContext` edits: none exist / none are built. The server-side credit gate is the ONLY gate, mirroring `lib/freemium.ts` (`CHAT_DAILY_FREE` precedent): add `FRIDGE_DAILY_FREE = 3` there, wire it per F-D2 (burst limit first, then daily gate for non-premium via `accountHasActivePremium` — note the repo helper is `accountHasActivePremium` on per-source subscriptions, NOT the draft's `hasActivePremium(account.subscription)`). 402 → iOS `APIError` path presents `PaywallStubView` exactly as the Clara chat screen does. All Task 2-4 references to UsageMeter/EntitlementStore/FreemiumLimits/PaywallView are struck.
> 3. **Log-write reuses the SHIPPED `MealLogService`** (`Clara/Features/Restaurants/MealLogService.swift`, T6b): promote it to `Clara/Features/MealLog/MealLogService.swift` (git mv, both existing call sites updated) and EXTEND the `MealLogging` protocol with `func logFridgeRecipe(_ input: FridgeLogInput) async throws -> LogResponseDTO` posting `{localDate, mealType, source:"FRIDGE", name, servings, perServing, fridgeRecipeId, clientRequestId}`. Task 2's `AddToLogService` is struck; its `MealLogWriteDTO`/`LogResponseDTO`/envelope-shape guidance applies to the new method's body/response types. (FRIDGE is caller-supplied-macros; RESTAURANT stays server-priced — both live in one service.)
> 4. **Cook tab: this cycle owns the rename** (Phase 3 has not landed): `RootTabView` `.fridge` → `case cook` (label "Cook", `systemImage: "frying.pan"`), body `CookView` = native segmented Picker "Scan" | "Fridge"; Scan segment shows the token-styled "Scan is coming soon" stub per the 2026-07-22 amendment item 1.
> 5. **Branches:** web = `fridge-backend` off current `main` (post-deploy tip); Clara = `phase4-fridge` off current `main` (`e1fd656`, includes the onboarding gate). Web merge to `main` auto-deploys production (Vercel git integration) — merge only at cycle end.
> 6. **Photo input: omitted entirely this cycle** (F-D5 default; Phase-3 capture does not exist). The route treats a supplied `imageUrl` as unusable: proceed chips-only and return `imageUsed: false` (fix 6 semantics); chips-only requests omit the field. Task 1 Step 5 is not built.
> 7. **Model + SDK call style: pin against the CURRENT `app/api/dish-checker/route.ts`** (model id, thinking flag, error taxonomy) rather than this doc's literals.
>
> **AMENDED 2026-07-22 (supersedes the specific steps it names; see the roadmap's two amendment blocks):**
>
> 1. **Tab wiring / Cook tab.** The shipped `RootTabView` (Clara `a466a68`) is `[restaurants, fridge, chat, stats, account]` with `selection = .restaurants` and **no `.scan` case** — Task 4's "preserve `selection: Tab = .scan`" instructions are void. Fridge mounts as the **Fridge segment of the Cook tab**: whichever of Phase 3/4 is built first renames `.fridge` → `case cook` (label "Cook", `systemImage: "frying.pan"`, body `CookView` = native segmented Picker "Scan" | "Fridge"). If Phase 4 builds first, this phase creates `CookView` with the Scan segment showing a token-styled "Scan is coming soon" stub (reuse `ScanPlaceholderView`'s content, then delete that file when Phase 3 lands).
> 2. **FridgeView is a mock, not a placeholder.** `Clara/Features/Fridge/FridgeView.swift` already ships the chips input card (add/remove chips, `FlowLayout`), a "Find recipes" primary button, and demo recipe result cards. Task 4 keeps this shipped layout, deletes the demo `FridgeRecipe` struct + `sampleRecipes`, and wires the VM/state machine (generating skeleton, results, valid-empty, error/retry, paywall) the mock lacks. **Reuse the existing private `FlowLayout` in `FridgeView.swift`** (promote it to `DesignSystem/FlowLayout.swift` unchanged) — do not create a second implementation.
> 3. **Branch base:** cut from the **current Clara `main` tip** (which includes Phase 3 if already landed — the `MealLogWriteDTO`/`AddToLogService` reuse then applies; otherwise this phase owns them per the stated fallback), not "the Phase-2 tip".
> 4. **Fix — parse/DTO strictness seam.** `parseFridgeRecipes` must guarantee every field `FridgeRecipeDTO` requires: coerce missing `description` → `""`, missing `mealType` → the request's `mealType` hint else `"dinner"`, missing `servings` → `1`; a recipe missing `name`, `steps`, `perServing`, or `fitsPlan` is **dropped** (macro/verdict fields are never invented). This keeps the iOS DTO non-optional without decode failures on lenient model output.
> 5. **Fix — allergen filter matching.** `applyAllergenFilter` matches banned terms with **word-boundary** regexes per term (mirroring the `lib/meal-plan.ts` allergy matcher and the `lib/diet-match.ts` extraction planned in `docs/restaurants/phase-1.md`), not raw substring — "butter" must not ban "butternut squash". Multi-word banned terms match as phrases.
> 6. **Fix — silent photo drop (F-D10).** When the server can't fetch/use `imageUrl`, it proceeds chips-only but returns `imageUsed: false` in the response; the client shows a non-blocking notice ("Couldn't read your photo — suggestions use your ingredient list"). No more invisible degradation.
> 7. The "Phase-2 edits are exactly two" claim is relaxed: the feature-keyed `UsageMeter` surface (`count(for:)/increment(for:)` + `.fridge` case) is a legitimate third edit if Phase 2 shipped without it — verify, then extend under test as Task 4 already assumes. REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. **Every task that creates or edits SwiftUI MUST invoke the `ui-ux-pro-max:ui-ux-pro-max` skill AND the `mobile-ios-design` skill before writing any Swift — this is a non-negotiable global user rule for all frontend work; it is restated in Step 1 of each such task.**
