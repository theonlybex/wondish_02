...
>
> 1. **Build order:** built **right after the Restaurants tab** (alongside Phase 5) — its backend is fully shipped. Branch from the **current Clara `main` tip** (post-Restaurants), not "the Phase-2 tip".
> 2. **Tab wiring.** The shipped `RootTabView` (Clara `a466a68`) has `selection = .restaurants` and **no `.scan` case** — Task 11's "keep `selection: Tab = .scan` (Scan remains the default tab)" is unimplementable and void; read it as **preserve `selection: Tab = .restaurants` and every other tab's body/label/systemImage unchanged**.
> 3. **StatsView is a mock, not a placeholder.** `Clara/Features/Stats/StatsView.swift` already ships the day selector (44-pt chevrons), calorie ring card, three macro tiles with capsule bars, a 7-day **BarMark** trend with dashed target `RuleMark`, and the today's-log card with an "Add meal" button. Tasks 8–9 keep this shipped card scaffolding and wire it: delete the demo models (`DayCalories`/`weekCalories`/`LoggedMeal`/`todaysLog`) and hardcoded `eaten`/`target`, and add the VM, locked/empty/error states, sheets, and undo the mock lacks. **The trend chart keeps the shipped BarMark form** (bars + dashed target rule; D9's partially-estimated days render at reduced opacity with the "Est." legend note) — the body's `LineMark CalorieTrendChart` spec is superseded; the `MacroDonutChart` is added per plan.
> 4. **Forward-compatible source badges.** The mock's log rows show a "Restaurant" source; `MealLogSource.RESTAURANT` does not exist server-side yet (it arrives with `docs/restaurants/phase-2.md`). The `MealLogDTO` source decode must map **unknown source strings to a `.other` case** (badge `.neutral`) so future enum values never fail the decode.
> 5. **Fix — fixture count.** Task 12's screenshot loop runs all **eight** fixture states — add the missing `statsProfileNotFound` launch; the "empty" and "profile-not-found" screens are distinct captures.
> 6. **Fix — zero target.** The D5 ring guard is `target > 0`, not just non-nil: `dayTarget.calories == 0` renders the D6 neutral ring (no division).
> 7. **Fix — delta-vs-outbox precedence.** A pulled delta row for a log that has a **pending un-dispatched** queued op does not overwrite the optimistic cache — the outbox wins locally until its op is dispatched, after which the server echo/delta reconciles. State this rule in Task 5 and cover it with the existing `test_deltaPull_upsert_byClientRequestId` plus one new test for the pending-op case.
> 8. **Fix — one persistence store.** `DeltaCursorState` (including `lastSyncSeed`) lives in the same versioned `FileOutboxStore` document as the ops (one file, one version field, one migration story) — not `UserDefaults`. `cancelQueuedDelete` runs on the sync-engine actor (same isolation as the drain) — state this explicitly as the race guard for F7 branch A. REQUIRED SUB-SKILL: Use `superpowers:subagent-driven-development` (recommended) or `superpowers:executing-plans` to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking. **Every task that creates or edits SwiftUI MUST invoke the `ui-ux-pro-max:ui-ux-pro-max` skill AND the `mobile-ios-design` skill before writing any Swift — this is a non-negotiable global user rule for all frontend work; it is restated in Step 1 of each such task.**
