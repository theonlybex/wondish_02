...
> **AMENDED 2026-07-22 (see the roadmap's amendment block):** Scan is **no longer the default tab** — Restaurants owns the first slot (Clara `c9a73d2`) and Scan re-enters inside a merged **Cook** tab when this phase is built. Everything in this plan about the Scan *feature* (capture → `/api/picture` → verdict → log) is unchanged; ignore its "default tab / zero taps from launch" framing and `selection = .scan` references.
>
> **AMENDED 2026-07-22 (second pass — concrete substitutions; supersedes the specific steps named):**
>
> 1. **Mount point / Cook tab (owns the unresolved question above).** The shipped `RootTabView` (Clara `a466a68`) has no `.scan` case; `ScanPlaceholderView` is unreachable dead code. Whichever of Phase 3/4 is built **first** creates the Cook container: rename the `.fridge` tab to `case cook`, label **"Cook"**, `systemImage: "frying.pan"`, body `CookView` — a top-aligned native segmented `Picker` ("Scan" | "Fridge") over the two feature roots. If Phase 3 builds first, the Fridge segment shows the existing `FridgeView` mock until Phase 4 wires it; if Phase 4 built first, `CookView` already exists and Task 5 here only fills the Scan segment. Task 5's "preserve `selection: Tab = .scan` / keep `Label("Scan", …)`" instructions and Task 6's "Scan is the default tab" screenshot assertions are **void**; screenshots launch with `-tab cook`. Delete `ScanPlaceholderView.swift`.
> 2. **Branch base:** cut from the **current Clara `main` tip** (mocks + any landed phases), not "the Phase-2 tip".
> 3. **Fix — confidence clamp direction (safety).** `parsePictureAnalysis`: an out-of-enum `verdict` is a parse failure (`null` → 422), never clamped; an out-of-enum `confidence` clamps to **`"low"`** (conservative — guarantees the P12 allergy downgrade fires rather than being silently skipped).
> 4. **Fix — retry charges usage.** A *successful* `retry()` after a failed attempt **does** charge the meter (one charge per delivered analysis; failed attempts stay free). This closes the free-quota loophole in the original "re-run analyze without re-charging usage" wording.
> 5. **Fix — distinguishable 429s.** The route returns `{"error":"rate_limited"}` for the burst limiter and `{"error":"daily_limit"}` for the daily backstop; the client maps them to distinct copy ("Too many scans — wait a moment" vs "You've hit today's scan limit — try again tomorrow") and keeps the 503 "took too long — try again" copy distinct from the generic unavailable message.
> 6. **Contract trim.** Drop the vestigial `mealType`/`localDate` request fields from `validatePictureInput` and the iOS request DTO — the route never reads them (meal type + date are chosen at *log* time and sent to `/api/meal-log`).
